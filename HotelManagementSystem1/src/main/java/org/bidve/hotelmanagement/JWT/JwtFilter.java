...
public class JwtFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain)
		...
		if(httpServletRequest.getServletPath().matches(regex: "/user/login|/user/forgotPassword|/user/signup")) {
			filterChain.doFilter(httpServletRequest, httpServletResponse);
		}else {
			String authorizationHeader = httpServletRequest.getHeader(s:"Authorization");
			String token = null;
			...
			if(userName != null && SecurityContextHolder.getContext().getAuthentication()==null) {
				...
				if(jwtUtil.validateToken(token,userDetails)) {
					...
					usernamePasswordAuthenticationTiken.setDetails(
